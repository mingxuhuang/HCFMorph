...
class Ourdata(Dataset):
    ...
    def __getitem__(self, index):
        ...
        CT1lab = sitk.GetArrayFromImage(CT1lab)
        MRlab = sitk.GetArrayFromImage(CT2lab)

        xlab = np.zeros_like(CT1lab)
        xlab = np.expand_dims(xlab, axis=0).repeat(4, axis=0)
        ylab = np.zeros_like(xlab)
        xlab[0] = (CT1lab==1).astype(np.uint8)
        xlab[1] = (CT1lab==2).astype(np.uint8)
        xlab[2] = (CT1lab==3).astype(np.uint8)
        xlab[3] = (CT1lab==4).astype(np.uint8)
