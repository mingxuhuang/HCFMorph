...
import numpy as np
import VMambaMorph.mambamorph.torch.configs_TransMorph2d as configs
from VMambaMorph.mambamorph.torch.networks import Unet, ConvBlock
import pdb
import sys
...
import layers
from mamba2D import MambaLayer
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.ape:
            ...
            absolute_pos_embed = nnf.interpolate(self.absolute_pos_embed, size=(Wh, Ww), mode='trilinear')
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        for i_layer in range(self.num_layers):
            layer = MambaLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               d_conv=d_conv,
                               expand=expand,
                               downsample=mamba2D.PatchMerging if (i_layer < self.num_layers - 1) else None)
            self.layers.append(layer)
...
class MambaCBlock(nn.Module):
    ...
    def forward(self, x, x_conv):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww, x_conv, xl = layer(x, Wh, Ww, x_conv)
            # if i in self.out_indices:
            #     norm_layer = getattr(self, f'norm{i}')
            #     x_out = norm_layer(x_out)
            #
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        for i_layer in range(self.num_layers):
            layer = VMambaDownLayer(dim=int(embed_dim * 2 ** i_layer),
                                    ...
                                    imgsize=(
                                        imgsize[0] // (4 * 2 ** i_layer), imgsize[1] // (4 * 2 ** i_layer)) if (
        ...
        for i_layer in range(self.num_layers):
            ...
            for i_layer in range(self.num_layers):
                ...
                layer = VMambaLayer(dim=int(embed_dim * 2 ** i_layer),
                                    ...
                                    expand=expand,
                                    downsample=PatchMerging if (i_layer < self.num_layers - 1) else None,
                                    imgsize=(imgsize[0] // (4 * 2 ** i_layer), imgsize[1] // (4 * 2 ** i_layer)) if (i_layer < self.num_layers) else (
                                    imgsize[0] // (4 * 2 ** (i_layer - 1)), imgsize[1] // (4 * 2 ** (i_layer - 1)),
                                    )
                                    )  # 32,192,320,
                self.layers.append(layer)
...
class MambaCMoMorphFeat(nn.Module):
    def __init__(self, config):
        ...
        self.reg_head1 = nn.Sequential(
            nn.Conv2d(embed_dim * 2, embed_dim * 2, 3, 1, padding=1),
            nn.Conv2d(embed_dim * 2, config.reg_head_chan, 1, 1),
            nn.Conv2d(config.reg_head_chan, config.reg_head_chan, 3, 1, padding=1),
            nn.InstanceNorm2d(config.reg_head_chan),
            nn.SiLU(),
            RegistrationHead(in_channels=config.reg_head_chan, out_channels=2, kernel_size=3, )
        )

        self.reg_head2 = nn.Sequential(
            nn.Conv2d(embed_dim, embed_dim, 3, 1, padding=1),
            nn.Conv2d(embed_dim, config.reg_head_chan, 1, 1),
            nn.Conv2d(config.reg_head_chan, config.reg_head_chan, 3, 1, padding=1),
            nn.InstanceNorm2d(config.reg_head_chan),
            nn.SiLU(),
            RegistrationHead(in_channels=config.reg_head_chan, out_channels=2, kernel_size=3, )
    ...
    def forward(self, source, target, return_pos_flow=True, return_feature=False):
        ...
        pos_flow1 = F.interpolate(pos_flow1, scale_factor=8, mode='bilinear', align_corners=False)
        pos_flow2 = F.interpolate(pos_flow2, scale_factor=4, mode='bilinear', align_corners=False)
        pos_flow3 = F.interpolate(pos_flow3, scale_factor=2, mode='bilinear', align_corners=False)
...
CONFIGS = {
    ...
    'MambaMorph': configs.get_2DMambaMorph_config(),
    'VMambaMorph': configs.get_2DVMambaMorph_config(),
    'RecVMambaMorph': configs.get_2DVMambaMorph_config(),
}
...
if __name__ =='__main__':
    ...
    source = torch.randn(1,1,256,256).cuda()
    target = torch.randn(1,1,256,256).cuda()
    model = MambaCMoMorphFeat(config).cuda()
    # print(model)
