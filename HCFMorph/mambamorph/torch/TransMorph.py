...
import numpy as np
import HCFMorph.mambamorph.torch.configs_TransMorph as configs
from HCFMorph.mambamorph.torch.networks import Unet, ConvBlock
import pdb
import sys
...
sys.path.append(r"D:\mx\CT_Re\VMambaMorph\mambamorph\torch")
import layers
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        for i_layer in range(self.num_layers):
            layer = MambaLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               d_conv=d_conv,
                               expand=expand,
                               downsample=mamba.PatchMerging if (i_layer < self.num_layers - 1) else None)
            self.layers.append(layer)
...
class MambaCBlock(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = MambaDownLayer(dim=int(embed_dim * 2 ** i_layer),
                               d_state=d_state,
                               d_conv=d_conv,
                               expand=expand,
                               downsample=PatchMerging2 if (i_layer < self.num_layers - 1) else None)
            self.layers.append(layer)
    ...
    def forward(self, x,x_conv):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, T, x, Wh, Ww, Wt,x_conv,xl = layer(x, Wh, Ww, Wt,x_conv)
            # if i in self.out_indices:
            #     norm_layer = getattr(self, f'norm{i}')
            #     x_out = norm_layer(x_out)
            #
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(MambaCBlock, self).train(mode)
        self._freeze_stages()

class VMambaCBlock(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        for i_layer in range(self.num_layers):
            layer = VMambaDownLayer(dim=int(embed_dim * 2 ** i_layer),
                               d_state=d_state,
                               d_conv=d_conv,
                               expand=expand,
                               downsample=PatchMerging2 if (i_layer < self.num_layers - 1) else None,
                                   ...
                                   imgsize=(
                                       imgsize[0] // (4 * 2 ** i_layer), imgsize[1] // (4 * 2 ** i_layer),
                                       imgsize[2] // (4 * 2 ** i_layer)) if (
                                           i_layer < self.num_layers) else (
                                       imgsize[0] // (4 * 2 ** (i_layer - 1)), imgsize[1] // (4 * 2 ** (i_layer - 1)),
                                       imgsize[2] // (4 * 2 ** (i_layer - 1))
                                   ))
            self.layers.append(layer)
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(VMambaCBlock, self).train(mode)
        self._freeze_stages()
...
class VMambaBlock(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        for i_layer in range(self.num_layers):
            self.layers = nn.ModuleList()
            for i_layer in range(self.num_layers):
                # imgsize = (32/(4*2**i_layer),192/(4*2**i_layer),320/(4*2**i_layer)) if (i_layer < self.num_layers - 1) else (32/(4*2**(i_layer-1)),192/(4*2**(i_layer-1)),320/(4*2**(i_layer-1)))
                # print(imgsize)
                layer = VMambaLayer(dim=int(embed_dim * 2 ** i_layer),
                                ...
                                expand=expand,
                                downsample=PatchMerging if (i_layer < self.num_layers - 1) else None,
                                imgsize=(imgsize[0]//(4*2**i_layer),imgsize[1]//(4*2**i_layer),imgsize[2]//(4*2**i_layer)) if (i_layer < self.num_layers) else (imgsize[0]//(4*2**(i_layer-1)),imgsize[1]//(4*2**(i_layer-1)),imgsize[2]//(4*2**(i_layer-1)))
                                    ) # 32,192,320,
                self.layers.append(layer)
...
class VxmFeat_model(nn.Module):
    ...
    def __init__(self,
        ...
        self.Re = VxmDense(inshape)
        # self.Re.unet_model.encoder[0][0].main = nn.Conv3d(32,16,kernel_size=3,padding=1,stride=1)
    ...
    def forward(self, source, target, return_pos_flow=True,
        ...
        y_source, pos_flow = self.Re(source, target, registration=True)
        return {'moved_vol': y_source, 'pos_flow': pos_flow}
...
class MambaCMorphFeat(nn.Module):
    def __init__(self, config):
        """
        MambaMorph Model
        """
        super(MambaCMorphFeat, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
        ...
        self.transformer = MambaCBlock(patch_size=config.patch_size,
                                      in_chans=nb_feat_extractor[-1][-1] * 2,
                                      embed_dim=config.embed_dim,
                                      depths=config.depths,
                                      drop_rate=config.drop_rate,
        ...
        self.c3 = nn.Sequential(
            Conv3dReLU(embed_dim // 2, config.embed_dim, 3, 1, use_batchnorm=False),
    ...
    def forward(self, source, target, return_pos_flow=True, return_feature=False):
        ...
        out_feats = self.transformer(x,self.c3(f4))
...
def getsize(imgsize,i):
    return [temp//i for temp in imgsize]



class MambaCMoMorphFeat(nn.Module):
    def __init__(self, config):
        """
        MambaMorph Model
        """
        super(MambaCMoMorphFeat, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
        ...
        self.reg_head1 = nn.Sequential(
            nn.Conv3d(embed_dim*2,embed_dim*2,3,1,padding=1),
            nn.Conv3d(embed_dim*2,config.reg_head_chan,1,1),
            nn.Conv3d(config.reg_head_chan,config.reg_head_chan,3,1,padding=1),
            nn.InstanceNorm3d(config.reg_head_chan),
            nn.SiLU(),
            RegistrationHead(in_channels=config.reg_head_chan, out_channels=3, kernel_size=3,)
        )

        self.reg_head2 = nn.Sequential(
            nn.Conv3d(embed_dim,embed_dim,3,1,padding=1),
            nn.Conv3d(embed_dim,config.reg_head_chan,1,1),
            nn.Conv3d(config.reg_head_chan,config.reg_head_chan,3,1,padding=1),
            nn.InstanceNorm3d(config.reg_head_chan),
            nn.SiLU(),
            RegistrationHead(in_channels=config.reg_head_chan, out_channels=3, kernel_size=3,)
        )
        self.reg_head3 = nn.Sequential(
            nn.Conv3d(embed_dim//2,embed_dim//2,3,1,padding=1),
            nn.Conv3d(embed_dim//2,config.reg_head_chan,1,1),
            nn.Conv3d(config.reg_head_chan,config.reg_head_chan,3,1,padding=1),
            nn.InstanceNorm3d(config.reg_head_chan),
        ...
        self.avg_pool = nn.AvgPool3d(3, stride=2, padding=1)
        self.integrate = layers.VecInt(config.img_size, 7)
        self.integrate1 = layers.VecInt(getsize(config.img_size,8), 7)
        self.integrate2 = layers.VecInt(getsize(config.img_size,4), 7)
        self.integrate3 = layers.VecInt(getsize(config.img_size,2), 7)
    ...
    def forward(self, source, target, return_pos_flow=True, return_feature=False):
        ...
        x = self.up1(out_feats[-1], f1)
        flow1 = self.reg_head1(x)
        x = self.up2(x, f2)
        flow2 = self.reg_head2(x)
        x = self.up3(x, f4)
        flow3 = self.reg_head3(x)
        x = self.up4(x, f5)
        flow = self.reg_head(x)
        pos_flow = self.integrate(flow)
        ...
        pos_flow1 = self.integrate1(flow1)
        pos_flow2 = self.integrate2(flow2)
        pos_flow3 = self.integrate3(flow3)


        pos_flow1 = F.interpolate(pos_flow1,scale_factor=8,mode='trilinear', align_corners=False)
        pos_flow2 = F.interpolate(pos_flow2,scale_factor=4,mode='trilinear', align_corners=False)
        pos_flow3 = F.interpolate(pos_flow3,scale_factor=2,mode='trilinear', align_corners=False)



        moved = self.spatial_trans(source, pos_flow)
        moved1 = self.spatial_trans(source, pos_flow1)
        moved2 = self.spatial_trans(source, pos_flow2)
        moved3 = self.spatial_trans(source, pos_flow3)


        ret = {'moved_vol': [moved,moved1,moved2,moved3], 'preint_flow': [flow,flow1,flow2,flow3]}
        if return_pos_flow:
            ret['pos_flow'] = [pos_flow,pos_flow1,pos_flow2,pos_flow3]
        if return_feature:
            for head in self.read_out_head:
                source_feat = head(source_feat)
                target_feat = head(target_feat)
...
class MambaCMoFusionMorphFeat(nn.Module):
    def __init__(self, config):
        """
        MambaMorph Model
        """
        super(MambaCMoFusionMorphFeat, self).__init__()
        if_convskip = config.if_convskip
        ...
        self.read_out_head.append(ConvBlock(len(config.img_size), nb_feat_extractor[-1][-1],
                                            nb_feat_extractor[-1][-1] // 2, stride=2))
        self.read_out_head.append(ConvBlock(len(config.img_size), nb_feat_extractor[-1][-1] // 2, 3))
        self.DF1 = DualPathFusion(embed_dim*4,base_resolution=base_resolution)#[2,2,2])
        self.DF2 = DualPathFusion(embed_dim * 2,base_resolution=base_resolution)#[4,4,4])
        self.DF3 = DualPathFusion(embed_dim * 1,base_resolution=base_resolution)#[4,4,4])
        self.DF4 = DualPathFusion(embed_dim //2,base_resolution=base_resolution)#[4,4,4])
        self.DF5 = DualPathFusion(config.reg_head_chan,base_resolution=base_resolution)#[4,8,8])

        self.transformer = MambaCBlock(patch_size=config.patch_size,
                                      in_chans=nb_feat_extractor[-1][-1],
                                      embed_dim=config.embed_dim,
                                      depths=config.depths,
                                      drop_rate=config.drop_rate,
                                      ape=config.ape,
        ...
        self.up4 = DecoderBlock(embed_dim // 2, config.reg_head_chan,
                                skip_channels=config.reg_head_chan if if_convskip else 0,
                                use_batchnorm=False)  # 384, 160, 160, 256
        self.c1 = Conv3dReLU(nb_feat_extractor[-1][-1], embed_dim // 2, 3, 1, use_batchnorm=False)
        self.c2 = Conv3dReLU(nb_feat_extractor[-1][-1], config.reg_head_chan, 3, 1, use_batchnorm=False)
        self.c3 = nn.Sequential(
            Conv3dReLU(embed_dim // 2, config.embed_dim, 3, 1, use_batchnorm=False),
            nn.AvgPool3d(3, stride=2, padding=1)
        )
        ...
        self.spatial_trans = SpatialTransformer(config.img_size)
        self.spatial_trans2 = SpatialTransformer(getsize(config.img_size,2))
        self.spatial_trans4 = SpatialTransformer(getsize(config.img_size,4))
        self.avg_pool = nn.AvgPool3d(3, stride=2, padding=1)
        self.integrate = layers.VecInt(config.img_size, 7)
        self.integrate1 = layers.VecInt(getsize(config.img_size,8), 7)
        self.integrate2 = layers.VecInt(getsize(config.img_size,4), 7)
    ...
    def forward(self, source, target, return_pos_flow=True, return_feature=False):
        ...
        if self.if_convskip:
            s_x_s0 = source_feat.clone()
            s_x_s1 = self.avg_pool(source_feat)
            s_f4 = self.c1(s_x_s1)
            s_f5 = self.c2(s_x_s0)

            t_x_s0 = target_feat.clone()
            t_x_s1 = self.avg_pool(target_feat)
            t_f4 = self.c1(t_x_s1)
            t_f5 = self.c2(t_x_s0)
        else:
            s_f4 = None
        ...
        s_out_feats = self.transformer(source_feat,self.c3(s_f4))
        t_out_feats = self.transformer(target_feat,self.c3(t_f4))
        ...
        if self.if_transskip:
            ...
            t_f1 = None
            t_f2 = None
        cf = self.DF1(t_out_feats[-1],s_out_feats[-1])
        f1 = self.DF2(t_f1,s_f1)
        x = self.up1(cf, f1)
        flow1 = self.reg_head1(x)
        pos_flow1 = self.integrate1(flow1)
        fe_flow1 = F.interpolate(pos_flow1,scale_factor=2,mode='trilinear', align_corners=False)
        s_f2 = self.spatial_trans4(s_f2,fe_flow1)
        f2 = self.DF3(t_f2,s_f2)
        x = self.up2(x, f2)
        flow2 = self.reg_head2(x)
        pos_flow2 = self.integrate2(flow2)
        fe_flow2 = F.interpolate(pos_flow2, scale_factor=2, mode='trilinear', align_corners=False)
        s_f4 = self.spatial_trans2(s_f4,fe_flow2)
        f4 = self.DF4(t_f4,s_f4)
        x = self.up3(x, f4)
        flow3 = self.reg_head3(x)
        pos_flow3 = self.integrate3(flow3)
        fe_flow3 = F.interpolate(pos_flow3, scale_factor=2, mode='trilinear', align_corners=False)
        s_f5 = self.spatial_trans(s_f5,fe_flow3)
        f5 = self.DF5(t_f5,s_f5)
...
class VMambaCMoFusionMorphFeat(nn.Module):
    def __init__(self, config):
        """
        MambaMorph Model
        """
        super(VMambaCMoFusionMorphFeat, self).__init__()
        if_convskip = config.if_convskip
        base_resolution = config.base_resolution
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        ...
        self.read_out_head.append(ConvBlock(len(config.img_size), nb_feat_extractor[-1][-1],
                                            nb_feat_extractor[-1][-1] // 2, stride=2))
        self.read_out_head.append(ConvBlock(len(config.img_size), nb_feat_extractor[-1][-1] // 2, 3))
        self.DF1 = DualPathFusion(embed_dim*4,base_resolution=base_resolution)
        self.DF2 = DualPathFusion(embed_dim * 2,base_resolution=base_resolution)
        self.DF3 = DualPathFusion(embed_dim * 1,base_resolution=base_resolution)
        self.DF4 = DualPathFusion(embed_dim //2,base_resolution=base_resolution)
        self.DF5 = DualPathFusion(config.reg_head_chan,base_resolution=base_resolution)

        self.transformer = VMambaCBlock(patch_size=config.patch_size,
                                      in_chans=nb_feat_extractor[-1][-1],
                                      embed_dim=config.embed_dim,
                                      depths=config.depths,
                                      drop_rate=config.drop_rate,
...
class MCMorphFeat(nn.Module):
    def __init__(self, config):
        """
        MambaMorph Model
        """
        super(MCMorphFeat, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
        ...
        self.up1 = MambaUpLayer(dim=embed_dim*2,d_state=config.d_state,d_conv=config.d_conv,expand=config.expand)
        self.up2 = MambaUpLayer(dim=embed_dim,d_state=config.d_state,d_conv=config.d_conv,expand=config.expand)
        self.up3 = MambaUpLayer(dim=embed_dim//2,d_state=config.d_state,d_conv=config.d_conv,expand=config.expand)
...
#     config.img_size = (32,192,320)
#     # config.in_chans = 3
#     # model = MambaCBlock(patch_size=config.patch_size,
#     #                                   in_chans=config.in_chans,
#     #                                   embed_dim=config.embed_dim,
#     #                                   depths=config.depths,
#     #                                   drop_rate=config.drop_rate,
...
#     #                                   d_state=config.d_state,
#     #                                   d_conv=config.d_conv,
#     #                                   expand=config.expand,).cuda()
#     # print("11")
#     source = torch.randn(1,1,32,192,320).cuda()
#     target = torch.randn(1,1,32,192,320).cuda()
#     model = MambaCMoFusionMorphFeat(config).cuda()
#     print(model)
