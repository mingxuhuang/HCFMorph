...
from HCFMorph.mambamorph.torch.networks import Unet
from HCFMorph.mambamorph.torch.vxmorph import VoxelMorph


from getourfile import getdirfile
from getdata import Ourdata,ACDCdata,ABDdata,IXIdata
import json
...
def getfiledir(pathfile,path = r'D:\mx\CT_Re\IXIdata\data'):
    alldir = []
    for i in pathfile:
        alldir.append([os.path.join(path,i,'T1_img.nii.gz'),os.path.join(path,i,'T2_img.nii.gz')])
    return alldir
...
with open(r"D:\mx\CT_Re\ACDCdata.json", 'r') as f:
    data = json.load(f)
train_subject = data['train']#getdirfile('train',5)#
val_subject = data['val']##getfiledir(data['val'])getdirfile('val')#
inshape = (32,192,192)#(32, 192, 320)#(128,128,128)
...
tttt = ['vm-feat','RDP','Corr','vimm-feat','HCFMorph']#['tm-feat','GM']
flowname = 'pos_flow'  #flowname = ['preint_flow','pos_flow']
for modelname in tttt[-1:]:
    print(modelname)
    ...
    model_dir = r'D:\mx\CT_Re\VMambaMorph\models'+args.model#args.model_dir
    ...
    if args.model == 'vm':
        ...
        model = TransMorph.VxmFeat_model(inshape=inshape,
        ...
        config.img_size = inshape#(32,160,160)#(32, 192, 320)
        ...
        config = CONFIGS_TM['MambaMorph']
        config.img_size = inshape#(32,160,160)#(32, 192, 320)
        config.base_resolution = [i//2 for i in inshape]#[32,32,32]
        model = TransMorph.MambaCMoFusionMorphFeat(config)
        if args.load_model:
            model.load_state_dict(torch.load(args.load_model))

    elif args.model == 'GM':
        GMimgshape = inshape#(32,160,160)#(32, 192, 320)
    ...
    model.to(device)
    transform_model = SpatialTransformer(inshape, mode='bilinear')  # STN(32,192,320)
    ...
    losses += [src_loss.Grad('l2', loss_mult=None).loss]#args.int_downsize
    weights += [args.weight]  # Regularization loss
    loss_weights = [1/2,1/6,1/6,1/6]
    ...
    reg_weight = 0.001
    lowest_loss = 1e3
    # trainset = Ourdata(train_subject, None,None)
    trainset = ACDCdata(train_subject, None,None)
    # trainset = IXIdata(train_subject, None, None)
    traindata = DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
    # valset = Ourdata(val_subject, None,None)
    valset = ACDCdata(val_subject, None,None)
    # valset = IXIdata(val_subject, None, None)
    valdata = DataLoader(valset, batch_size=args.batch_size, shuffle=False)
    # TODO: set optimizer
    ...
    for epoch in range(args.initial_epoch, args.epochs):
        ...
        for step,tdata in enumerate(traindata):
            # vols, segs = next(generator)
            zero_flow = np.zeros(
                (args.batch_size, *tuple([int(tmp / args.int_downsize) for tmp in inshape]), len(inshape)))
            zero_flow = torch.from_numpy(zero_flow).to(device).float().permute(0, 4, 1, 2, 3)
            # inputs = [vols[0], vols[1]]  # src_img, tgt_img
            # y_true = [segs[1], zero_flow]  # tgt_label, 0
            # src_label = segs[0]
            inputs = [tdata[0], tdata[1]]  # src_img, tgt_img
            y_true = [tdata[3], zero_flow]  # tgt_label, 0
            src_label = tdata[2]
            ...
            with conditional_autocast(not args.no_amp):
                ...
                preint_flow = ret_dict[flowname]#ret_dict[preint_flow'pos_flow']# 有pos用pos，没有用pare


                # pos_flow = ret_dict['pos_flow']
                # warped_vol, preint_flow, pos_flow = model(*inputs, return_both_flow=True)
                if type(warped_vol) == list:
                    warped_label = [transform_model(src_label, preint_flow[temp]) for temp in range(len(preint_flow))]
                else:
                ...
                if type(warped_vol) == list:
                    for temp in range(len(warped_vol)):
                        loss_temp = 0
                        y_pred = (warped_label[temp], preint_flow[temp])
                        for n, loss_function in enumerate(losses):
                            # curr_loss = loss_function(y_true[n], y_pred[n], ignore_label=label_ignore)
                            if n == 1 and 'smooth' in ret_dict.keys():
                                curr_loss = ret_dict['smooth']
                            else:
                            ...
                            loss_temp+=curr_loss
                        loss += loss_temp*loss_weights[temp]
                else:
                ...
                if args.cl > 0:
                    ...
                    cl_loss = cl_loss_fn.loss(feat, src_label, y_true[0], ignore_label=None)
        ...
        with torch.no_grad():
            ...
            for val_step,vdata in enumerate(valdata):
                ...
                y_true = [vdata[3], zero_flow]  # tgt_label, 0
                src_label = vdata[2]
                # src_label = torch.from_numpy(src_label).to(device).float().permute(0, 4, 1, 2, 3)
                # inputs = [torch.from_numpy(d).to(device).float().permute(0, 4, 1, 2, 3) for d in inputs]  # volume pairs
                # y_true = [torch.from_numpy(d).to(device).float().permute(0, 4, 1, 2, 3) for d in y_true]
                ...
                with conditional_autocast(not args.no_amp):
                    ...
                    warped_vol = ret_dict['moved_vol']
                    preint_flow = ret_dict[flowname]#ret_dict['preint_flowpos_flow']# 有pos用pos，没有用pare
                    # pos_flow = ret_dict['pos_flow']
                    if type(warped_vol) == list:
                        warped_label = [transform_model(src_label, preint_flow[temp]) for temp in range(len(preint_flow))]
                    else:
                    ...
                    loss_list_val = []
                    if type(warped_vol) == list:
                        y_pred = (warped_label[0], preint_flow[0])
                        for n, loss_function in enumerate(losses):
                            if n == 1 and 'smooth' in ret_dict.keys():
                                curr_loss = ret_dict['smooth']
                            else:
                        ...
                        #             loss_list_val.append(curr_loss.item())
                        #         loss_temp += curr_loss
                        #     loss_val += loss_temp*loss_weights[temp]
                    else:
                        y_pred = (warped_label, preint_flow)
                        for n, loss_function in enumerate(losses):
                            if n == 1 and 'smooth' in ret_dict.keys():
        ...
        print('-' * 5 + f'This epoch takes {time_consumption} min. Next lr={currentlr} \n')
        train_rec.close()
        val_rec.close()
